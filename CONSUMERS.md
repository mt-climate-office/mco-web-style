...
- **Deferred to the next kit release:** `core/mco-core.js` still names
  ...
  comment. Fold it into the next release, when the hashes are regenerated anyway.
- **Kit defect, needs a release:** `MCO.shiftDate` anchors at local noon (right)
  ...
  the version mesonet-photos keeps locally, and deliberately does NOT call
