...
| mco-mesonet-photos | Vanilla MapLibre SPA (**`docs/` is the Pages root** — the app is `docs/index.html`, ~1,670 lines with one inline module) | **Next up (2026-08).** Deploy caution ×2: Pages publishes `/docs` on `main` AND the page is proxied at `mesonet.climate.umt.edu/photos/` (mesonet_app Caddyfile) — push = production on both URLs. The repo has its own Actions (`mirror_photos.yml` runs ~12×/day and **commits `docs/preview.png` back to main** — pull/rebase before pushing; `adjust_dst.yml` edits crons on DST Sundays). Duplication to delete: token block, control polish, navbar/toast/tooltip/modal CSS, `basemapStyleUrl`/`syncThemeIcons`/`lsSet`/`showToast`, overlay paints + tribal label shortenings, fit control, snapback (code comments literally say "shared with the Status map"). Bugs to fix in-flight: focus restore does `.focus(); .blur()` (≈ docs/index.html:1324 — drops focus to body); `/` shortcut lacks `?kbd=off` (≈:1458); `mco-info-seen` localStorage key is unprefixed (shim to `mco-photos-*`); README + two code comments still say the app is D3 (it's MapLibre); dead assets `docs/mco_logo.png` + `docs/Mesonet_Logo.png` (delete); `terraform/terraform.tfvars.example` carries the repo's old name. Keeps app-local: the photo-mosaic machinery (`coverCropToDataURL` + LRU cache, per-station image sources), gallery/lightbox dialogs, date stepper with hold-to-repeat, direction segments + `<select>` fallback, `updateSocialMeta`, and the branded PNG export (kit export module doesn't exist yet — don't extract). CSP endpoints: `mesonet.climate.umt.edu` in `connect-src` AND `img-src` (WebP/JPG photos), CARTO hosts, `s3.amazonaws.com` if hillshade is adopted. **A11y baseline is thinner than status's was** — absent today: skip link, `<main>`, any live region beyond `#toast`, and an sr-table twin (proposed twin: one row per grid cell — station name/ID, direction shown, photo timestamp in MT, or "no photo"). MapLibre tags are pinned 5.18.0 but carry **no `integrity`** — add SRI. ⚠️ Its preview automation (`scripts/generate_preview.py`, run ~12×/day by CI) drives the live page headlessly via `?export=dark` + `#btn-export` and a 4 s timing assumption — run it locally against the migrated page before deploying (MIGRATING.md § Verification). Decisions to ASK (not settled): hillshade on a photo-mosaic map (may fight the imagery); counties (it has a Boundaries toggle already — integrate, don't duplicate); its ≤750px full-brand shed vs status's keep-branding choice. |
